...
test.describe('Tool - Text to ASCII binary', () => {
  ...
  test('Text to binary conversion', async ({ page }) => {
    await page.getByTestId('text-to-binary-input').fill('dev-tools');
    const binary = await page.getByTestId('text-to-binary-output').inputValue();

    expect(binary).toEqual('01100100 01100101 01110110 00101101 01110100 01101111 01101111 01101100 01110011');
  });

  test('Binary to text conversion', async ({ page }) => {
    await page.getByTestId('binary-to-text-input').fill('01100100 01100101 01110110 00101101 01110100 01101111 01101111 01101100 01110011');
    const text = await page.getByTestId('binary-to-text-output').inputValue();

    expect(text).toEqual('dev-tools');
  });
});
